...
- a dataframe with columns as described above.
...
Note that pdf.width and pdf.height, if not specified, will be calculated and the pdf size
necessary to print requested linkage groups will be used.
...
```{r}
 outfile = paste(tempdir(),"/hyper_dupnbr.pdf",sep="")
...
will be italic (3).
```{r}
 outfile = paste(tempdir(),"/hyper_showonly.pdf",sep="")
...
```{r}
 outfile = paste(tempdir(),"/hyper_titles.pdf",sep="")
...
1. **qtldf** = a dataframe containing these columns:
...
2. **qtlscanone** = scanone object from package r/qtl. If provided, all QTLs in the dataframe will be drawn by calculating their start and end with the r/qtl function bayesint with defaults.
...
- **lg.lwd** - linkage group linewidth. The width of the line around the chromosome.  Defaults to par("lwd").
...
outfile = paste(tempdir(),"/oat_denmap.pdf",sep="")
